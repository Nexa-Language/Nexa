...
__tool_calculate_schema = {
    "name": "calculate",
    "description": "Perform a calculation given a mathematical expression",
    "parameters": {
        "type": "object",
        "properties": {
        "expression": {
...
def calculate(expression):
    """Perform a calculation given a mathematical expression"""
...
Assistant = NexaAgent(
    ...
    tools=[__tool_calculate_schema]
)

def flow_main():
    _std_ui_banner("Nexa Agent Template", "Type your question, or /exit to quit.")
    # v2.0 autoloop: ReAct cycle (E-dimension)
    _kernel = get_kernel()
    _config = AutoLoopConfig(max_steps=50, exit_when="user typed /exit", timeout=600)
    _step_count = 0
    _start_time = time.time() if _config.timeout else 0
    while _step_count < _config.max_steps:
        _step_count += 1
        if _config.timeout and (time.time() - _start_time) > _config.timeout:
            print(f'autoloop timeout after {_config.timeout}s')
            break
        user_input = _std_ui_input("You> ")
        if user_input == "/exit":
            print("Goodbye!")
            sys.exit(0)
        # v2.0 with_context: context scope
        _cm = ContextManager(kernel=get_kernel())
        _cm.enter_scope('scope', {'max_tokens': 100000, 'strategy': 'sliding_window'})
        result = Assistant.run(user_input)
        _cm.exit_scope('scope')
        # v2.0 verify
        _ei = EvaluationInterface()
        _vr = _ei.verify_satisfies(result, "string")
        if not _vr.passed and _vr.correction_hint:
            print(f'[verify] FAIL: {_vr.correction_hint}')
        _std_ui_agent_reply("Assistant", result)
    # autoloop completed
    return result
