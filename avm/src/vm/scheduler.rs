...
mod tests {
    ...
    fn test_smart_scheduler_submit() {
        ...
        let info = AgentScheduleInfo::new(1).with_priority(100);
    ...
    fn test_rebalance() {
        ...
        assert!(adjustments.len() <= scheduler.config.max_concurrent_agents);
    ...
    fn test_tree_of_thoughts_with_cow_and_workstealing() {
        ...
        memory.set(
            "thought".to_string(),
            crate::vm::cow_memory::MemoryValue::String("approach_A".to_string()),
        );

        memory.switch_to_snapshot(branch2);
        memory.set(
            "thought".to_string(),
            crate::vm::cow_memory::MemoryValue::String("approach_B".to_string()),
        );

        memory.switch_to_snapshot(branch3);
        memory.set(
            "thought".to_string(),
            crate::vm::cow_memory::MemoryValue::String("approach_C".to_string()),
        );
