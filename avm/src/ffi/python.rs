...
impl PythonModule {
    ...
    pub fn new(name: &str) -> Self {
        Self {
            name: name.to_string(),
            version: env!("NEXA_VERSION").to_string(),
...
impl AvmPy {
    ...
    pub fn parse(&self, source: &str) -> Result<serde_json::Value, String> {
        let program =
            Parser::parse_from_source(source).map_err(|e| format!("Parse error: {}", e))?;
...
mod pyo3_bindings {
    ...
    /// Python 模块定义
    #[pymodule]
    fn nexa_avm(m: &Bound<'_, PyModule>) -> PyResult<()> {
        m.add_class::<PyAvm>()?;
        m.add_class::<PyValue>()?;
        m.add_function(wrap_pyfunction!(compile, m)?)?;
        m.add_function(wrap_pyfunction!(run, m)?)?;
        m.add("__version__", env!("NEXA_VERSION"))?;
        Ok(())
    }
...
mod tests {
    ...
    fn test_python_value_list() {
        let val = PythonValue::list_val(vec![PythonValue::int_val(1), PythonValue::int_val(2)]);
